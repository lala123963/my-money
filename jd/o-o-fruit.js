...
async function doDailyTask() {
  ...
  if (!$.farmTask.signInit.todaySigned) {
    ...
    if ($.signResult.code === "0") {
      console.log(`【签到成功】获得${$.signResult.amount}gߒ眜n`)
      //message += `【签到成功】获得${$.signResult.amount}gߒ眮`//连续签到${signResult.signDay}天
    } else {
      // message += `签到失败,详询日志\n`;
  ...
  if ($.farmInfo.todayGotWaterGoalTask.canPop) {
    ...
    if ($.goalResult.code === '0') {
      console.log(`【被水滴砸中】获得${$.goalResult.addEnergy}gߒ眜n`);
      // message += `【被水滴砸中】获得${$.goalResult.addEnergy}gߒ眮`
    }
  }
  ...
  if ($.farmTask.gotBrowseTaskAdInit.f) {
    ...
    if (browseFail > 0) {
      console.log(`【广告浏览】完成${browseSuccess}个,失败${browseFail},获得${browseReward}gߒ眜n`);
      // message += `【广告浏览】完成${browseSuccess}个,失败${browseFail},获得${browseReward}gߒ眮`;
    } else {
      console.log(`【广告浏览】完成${browseSuccess}个,获得${browseReward}gߒ眮`);
      // message += `【广告浏览】完成${browseSuccess}个,获得${browseReward}gߒ眮`;
    }
  }
  //定时领水
  if (!$.farmTask.gotThreeMealInit.f) {
    ...
    if ($.threeMeal.code === "0") {
      console.log(`【定时领水】获得${$.threeMeal.amount}gߒ眮`);
      // message += `【定时领水】获得${$.threeMeal.amount}gߒ眮`;
    } else {
...
async function predictionFruit() {
  ...
  message += `【剩余 水滴】${$.farmInfo.farmUserPro.totalEnergy}gߒ眮`;
  message += `【水果ߍ騿륺检᤻(($.farmInfo.farmUserPro.treeEnergy / $.farmInfo.farmUserPro.treeTotalEnergy) * 100).toFixed(2)}%，已浇水${$.farmInfo.farmUserPro.treeEnergy / 10}次,还需${($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy) / 10}次\n`
  ...
  message += `【预测】${waterD === 1 ? '明天' : waterD === 2 ? '后天' : waterD + '天之后'}(${timeFormat(24 * 60 * 60 * 1000 * waterD + Date.now())}日)可兑换水果ߍ頍
}
//浇水十次
...
async function getFirstWaterAward() {
  ...
  if (!$.farmTask.firstWaterInit.f && $.farmTask.firstWaterInit.totalWaterTimes > 0) {
    ...
    if ($.firstWaterReward.code === '0') {
      console.log(`【首次浇水奖励】获得${$.firstWaterReward.amount}gߒ眮`);
      // message += `【首次浇水奖励】获得${$.firstWaterReward.amount}gߒ眮`;
    } else {
...
async function getTenWaterAward() {
  ...
  if (!$.farmTask.totalWaterTaskInit.f && $.farmTask.totalWaterTaskInit.totalWaterTaskTimes >= $.farmTask.totalWaterTaskInit.totalWaterTaskLimit) {
    ...
    if ($.totalWaterReward.code === '0') {
      console.log(`【十次浇水奖励】获得${$.totalWaterReward.totalWaterTaskEnergy}gߒ眮`);
      // message += `【十次浇水奖励】获得${$.totalWaterReward.totalWaterTaskEnergy}gߒ眮`;
    } else {
...
function gotStageAward() {
  return new Promise(async resolve => {
    if ($.waterResult.waterStatus === 0 && $.waterResult.treeEnergy === 10) {
      ...
      if ($.gotStageAwardForFarmRes.code === '0') {
        // message += `【果树开花了】奖励${$.gotStageAwardForFarmRes.addEnergy}gߒ眮`;
        console.log(`【果树开花了】奖励${$.gotStageAwardForFarmRes.addEnergy}gߒ眮`);
      }
      ...
      if ($.gotStageAwardForFarmRes.code === '0') {
        // message += `【果树结果了】奖励${$.gotStageAwardForFarmRes.addEnergy}gߒ眮`;
        console.log(`【果树结果了】奖励${$.gotStageAwardForFarmRes.addEnergy}gߒ眮`);
      }
    }
    resolve()
  })
...
async function masterHelpShare() {
  ...
  if (helpSuccessPeoples && helpSuccessPeoples.length > 0) {
    message += `【您助力的好友ߑ죀᤻helpSuccessPeoples.substr(0, helpSuccessPeoples.length - 1)}\n`;
  }
  if (salveHelpAddWater > 0) {
    // message += `【助力好友ߑ죀ᨎ祾礻salveHelpAddWater}gߒ眮`;
    console.log(`【助力好友ߑ죀ᨎ祾礻salveHelpAddWater}gߒ眮`);
  }
  message += `【今日剩余助力ߑ죀᤻remainTimes}次\n`;  
}
//水滴雨
  ...
  if ($.clockInInit.code === '0') {
    ...
    if (!$.clockInInit.todaySigned) {
      ...
      if ($.clockInForFarmRes.code === '0') {
        // message += `【第${$.clockInForFarmRes.signDay}天签到】获得${$.clockInForFarmRes.amount}gߒ眮`;
        console.log(`【第${$.clockInForFarmRes.signDay}天签到】获得${$.clockInForFarmRes.amount}gߒ眮`)
        if ($.clockInForFarmRes.signDay === 7) {
          //可以领取惊喜礼包
          console.log('开始领取--惊喜礼包38g水滴');
          await gotClockInGift();
          if ($.gotClockInGiftRes.code === '0') {
            // message += `【惊喜礼包】获得${$.gotClockInGiftRes.amount}gߒ眮`;
            console.log(`【惊喜礼包】获得${$.gotClockInGiftRes.amount}gߒ眮`);
          }
        }
      }
    }
